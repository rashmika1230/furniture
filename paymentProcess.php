...
$merchantId = "1224009";
$merchantSecret = "MjgyMDQ5NjUyOTQwNDA4MzQwODI0MjA0ODgzMzIyNjgwOTk5MTk0";
$items = "";
$netTotal = 0;
$currency = "LKR";
$orderId = uniqid();
...
for ($i = 0; $i < sizeof($stockList); $i++) {
    ...
    `product` ON 
    stock.product_id = product.id WHERE
    stock.stock_id = '" . $stockList[$i] . "' ");
    ...
    if ($stockQty >= $qtyList[$i]) {
        ...
        echo ("Product has No availibale in stock");
    }
...
$payment["address"] = $user["no"] . "," . $user["line1"] . "," . $user["line1"];
